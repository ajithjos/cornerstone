...
use super::shared::{
    ActivityRng, build_generated_activity, generate_unique_items, integer_item,
...
pub(super) fn generate(runtime: &MaterialRuntime, seed: u64) -> anyhow::Result<GeneratedActivity> {
    ...
    let modes = parameter_string_list(runtime, "modes")
        .unwrap_or_else(|| vec!["double".to_string(), "add_10".to_string()]);

...
deploy_dev_resolve_tokens() {
	local value="$1"
	value="${value//__CORNERSTONE_DEV_SITE_ROOT__/$CORNERSTONE_DEV_SITE_ROOT}"
...
deploy_dev_init() {
	...
	DEPLOY_DEV_SITE_DIR="${DEPLOY_DEV_SITE_DIR:-$DEPLOY_DEV_REPO_ROOT/dev/site_root}"
	DEPLOY_DEV_CONTROL_DIR="${DEPLOY_DEV_CONTROL_DIR:-$DEPLOY_DEV_SITE_DIR/control}"
	DEPLOY_DEV_SECRETS_DIR="${DEPLOY_DEV_SECRETS_DIR:-$DEPLOY_DEV_SITE_DIR/secrets}"
...
deploy_dev_load_config() {
	...
	if [[ -z "$DEPLOY_DEV_SITE_DIR" ]]; then
		DEPLOY_DEV_SITE_DIR="$(deploy_resolve_path "$DEPLOY_DEV_REPO_ROOT" "$(deploy_trim "$(setup_env_get CORNERSTONE_DEV_SITE_ROOT)")")"
	fi
	if [[ -z "$DEPLOY_DEV_SITE_DIR" ]]; then
		DEPLOY_DEV_SITE_DIR="$DEPLOY_DEV_REPO_ROOT/dev/site_root"
	fi
	DEPLOY_DEV_CONTROL_DIR="${DEPLOY_DEV_CONTROL_DIR:-$DEPLOY_DEV_SITE_DIR/control}"
	DEPLOY_DEV_SECRETS_DIR="${DEPLOY_DEV_SECRETS_DIR:-$DEPLOY_DEV_SITE_DIR/secrets}"
	...
	export CORNERSTONE_DEV_SITE_ROOT="$(deploy_trim "$(setup_env_get CORNERSTONE_DEV_SITE_ROOT)")"
	...
	CORNERSTONE_DEV_SITE_ROOT="$(deploy_dev_resolve_tokens "$CORNERSTONE_DEV_SITE_ROOT")"
	...
	export CORNERSTONE_DEV_SITE_ROOT
...
deploy_dev_ensure_dirs() {
	mkdir -p \
		...
		"$DEPLOY_DEV_REPO_ROOT/dev/scratchpad" \
